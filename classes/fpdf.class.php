...
function sizeOfText( $texte, $largeur )
{
	...
	while ( $loop )
	{
		...
		$res = 1 + ($largeur>0?floor( $length / $largeur):0) ;
		$nb_lines += $res;
	}
	return $nb_lines;
}
...
function addSociete($logo, $nom, $adresse )
{
	$x1 = 10;
	$y1 = 8;
	...
	if(file_exists($logo) && $logo!='../images/'){
	$this->Image($logo,10,8,0,0);
...
function etatcom($tva)
{
	...
	$this->Line( $r1, $mid, $r2, $mid);
	$this->SetXY( $r1 + 16 , $y1+1 );
	$this->Cell(40, 4, "Etat des commissions", '', '', "C");
	$this->SetFont( "Helvetica", "", 10);
	$this->SetXY( $r1 + 16 , $y1+5 );
	$this->Cell(40, 5, $tva, '', '', "C");
}
